      ...
      {props.certificate.map((cert) =>
        <div className='item' key={cert.name}>
          ...
          <div style={{position:'relative', display:'inline-flex' }}>
            ...
            {cert.certificate==='diplomaNfts'?<a style={{width:'150px'}} href={diplomaNfts} target="_blank" rel="noreferrer"><FaRegPlusSquare size={30} /></a>
            :<a style={{width:'150px'}} href={diplomaDefi} target="_blank" rel="noreferrer"><FaRegPlusSquare size={30} /></a>}
            </div>
        </div>
      )}
    </div>
